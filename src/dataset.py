...
class ARDataset_without_ref(Dataset):
    def __init__(self, dataset_path, augmentation=None, \
                 augmentation_images=None, preprocessing=None, is_train=True, ):
        """Dataset parameters initialization
        ...
        noshadow_path = os.path.join(dataset_path, 'noshadow1')#2
        ...
        # files_names_list = sorted(os.listdir(noshadow_path))
        files_names_list = sorted(os.listdir(mask_path))
    ...
    def __getitem__(self, i):
        ...
        # bg_mask = cv2.imread('/datadrive1/automobiles/data/bg_mask.jpg',0)
        # bg_mask=cv2.resize(bg_mask,(w,h),interpolation = cv2.INTER_CUBIC)

        if self.is_train:
            # result image
            res_image = cv2.imread(self.shadow_paths[i])
            res_image = cv2.cvtColor(res_image, cv2.COLOR_BGR2RGB)
            # res_image=cv2.resize(res_image,(w,h),interpolation = cv2.INTER_CUBIC)

           
            # apply augmentations for images
            if self.augmentation_images:
                sample = self.augmentation_images(image=image, image2=res_image)
                image = sample['image']
                res_image = sample['image2']
            # collect images for augmentations
            # mask = np.stack([mask,bg_mask], axis=-1).astype('float')
        ...
        image, mask = image.astype(np.float) / 255.0, \
        							mask.astype(np.float) / 255.0
        # mask_o=mask_o.astype(np.float) / 127.5 - 1.0
...
class ARDataset_with_ref(Dataset):
    def __init__(self, dataset_path, augmentation=None, \
                 augmentation_images=None, preprocessing=None, is_train=True, ):
        """Dataset parameters initialization
        ...
        noshadow_path = os.path.join(dataset_path, 'noshadow1')#2
        mask_path = os.path.join(dataset_path, 'masks')
        ref_img_path = os.path.join(dataset_path, 'ref_img')
        ref_mask_path = os.path.join(dataset_path, 'ref_mask')
        ref_mask_shadow_path = os.path.join(dataset_path, 'ref_mask_shadow')


        # collect paths to images
        self.noshadow_paths = []; self.mask_paths = [];
        self.shadow_paths = []; self.ref_img_paths=[]; self.ref_mask_paths=[];
        self.ref_shadow_mask_paths=[];


        self.black_image = np.zeros((1080,1920,1), np.uint8)
        ...
        ref_files_names_list = sorted(os.listdir(ref_img_path))


        for file_name in ref_files_names_list:
            self.ref_img_paths.append(os.path.join(ref_img_path, file_name))
            self.ref_mask_paths.append(os.path.join(ref_mask_path, file_name))
            self.ref_shadow_mask_paths.append(os.path.join(ref_mask_shadow_path, file_name))
    ...
    def __getitem__(self, i):
        ...
        # mask for inserted object
        mask = cv2.imread(self.mask_paths[i],0)
        mask=cv2.hconcat([mask,self.black_image])   #mask
        random_i=randint(0,5)

        ref_image = cv2.imread(self.ref_img_paths[random_i])
        ref_image = cv2.cvtColor(ref_image, cv2.COLOR_BGR2RGB)

        image=cv2.hconcat([image,ref_image])    #noshadow
        
        robject_mask=cv2.imread(self.ref_mask_paths[random_i],0)
        robject_mask=cv2.hconcat([self.black_image,robject_mask])    #robject_mask

        rshadow_mask=cv2.imread(self.ref_shadow_mask_paths[random_i],0)
        rshadow_mask=cv2.hconcat([self.black_image,rshadow_mask])    #robject_mask
        ...
        if self.is_train:
            ...
            res_image = cv2.imread(self.shadow_paths[i])
            res_image = cv2.cvtColor(res_image, cv2.COLOR_BGR2RGB)
            res_image=cv2.hconcat([res_image,ref_image])  #shadow image
            # res_image=cv2.resize(res_image,(w,h),interpolation = cv2.INTER_CUBIC)
